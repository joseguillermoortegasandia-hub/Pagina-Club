...
-- ============================================================
-- EXTENSIONES NEXT.JS / MULTI-TENANT / CHAT EN VIVO
-- ============================================================
...
create index if not exists club_domains_club_idx on public.club_domains(club_id);
alter table public.club_domains enable row level security;
drop policy if exists club_domains_read on public.club_domains;
create policy club_domains_read on public.club_domains for select to authenticated
using (club_id=public.current_user_club_id());
drop policy if exists club_domains_admin on public.club_domains;
create policy club_domains_admin on public.club_domains for all to authenticated
using (club_id=public.current_user_club_id() and public.can_manage_club())
with check (club_id=public.current_user_club_id() and public.can_manage_club());
...
create index if not exists live_chat_stream_idx on public.live_chat_messages(stream_id,created_at desc);
alter table public.live_chat_messages enable row level security;
drop policy if exists live_chat_read on public.live_chat_messages;
create policy live_chat_read on public.live_chat_messages for select to authenticated
using (club_id=public.current_user_club_id());
drop policy if exists live_chat_insert on public.live_chat_messages;
create policy live_chat_insert on public.live_chat_messages for insert to authenticated
with check (club_id=public.current_user_club_id() and user_id=auth.uid());
drop policy if exists live_chat_delete on public.live_chat_messages;
create policy live_chat_delete on public.live_chat_messages for delete to authenticated
using (club_id=public.current_user_club_id() and (user_id=auth.uid() or public.can_edit_content()));

-- Realtime: ejecutar de forma idempotente si las tablas aún no están en la publicación.
do $$ begin
...
-- Índices adicionales para las consultas más frecuentes del portal.
create index if not exists teams_club_sport_idx on public.teams(club_id,sport_id);
create index if not exists players_club_team_idx on public.players(club_id,team_id);
create index if not exists standings_club_sport_idx on public.standings(club_id,sport_id,rank);
create index if not exists news_club_published_idx on public.news(club_id,published_at desc);
create index if not exists activities_club_starts_idx on public.activities(club_id,starts_at);

-- Presidentes de liga pueden almacenar archivos; las tablas de destino siguen limitadas por sus RLS.
drop policy if exists club_media_insert on storage.objects;
create policy club_media_insert on storage.objects for insert to authenticated with check (
  bucket_id='club-media' and (storage.foldername(name))[1]=public.current_user_club_id()::text
  and public.current_user_role() in ('super_admin','club_admin','league_president','editor','manager')
);
drop policy if exists club_media_update on storage.objects;
create policy club_media_update on storage.objects for update to authenticated using (
  bucket_id='club-media' and (storage.foldername(name))[1]=public.current_user_club_id()::text
  and public.current_user_role() in ('super_admin','club_admin','league_president','editor','manager')
);
drop policy if exists club_media_delete on storage.objects;
create policy club_media_delete on storage.objects for delete to authenticated using (
  bucket_id='club-media' and (storage.foldername(name))[1]=public.current_user_club_id()::text
  and public.current_user_role() in ('super_admin','club_admin','league_president','editor','manager')
);

-- Endurecimiento de privacidad: un socio solo lee su propio perfil completo.
-- Los administradores de club sí pueden listar perfiles para gestión.
drop policy if exists profiles_read on public.profiles;
create policy profiles_read on public.profiles for select to authenticated using (
  id=auth.uid() or (club_id=public.current_user_club_id() and public.can_manage_club())
);

alter table public.streams add column if not exists latency_mode text not null default 'reduced';
do $$ begin
  alter table public.streams add constraint streams_latency_mode_check check (latency_mode in ('standard','reduced','low'));
exception when duplicate_object then null; end $$;
